...
RUN apt-get update && export DEBIAN_FRONTEND=noninteractive \
    && apt-get -y install --no-install-recommends curl git python3 python3-pip && pip3 install autogen && pip install autogen-core==0.4.0.dev3 && pip install cryptography && pip install PyJWT && pip install rsa
